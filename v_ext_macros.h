...
/*Custom Instruction start*/
#define VI_SEGMENT_MASK_VARS \
  const int seg_midx = fn / 64; \
  const int seg_mpos = fn % 64;

#define VI_LOOP_SEGMENT_SKIP(BODY) \
  VI_SEGMENT_MASK_VARS \
  if (insn.v_vm() == 0) { \
    BODY; \
    bool skip = ((P.VU.elt<uint64_t>(0, seg_midx) >> seg_mpos) & 0x1) == 0; \
    if (skip) { \
        continue; \
    } \
  }

#define VI_SEGMENT_SKIP \
  VI_LOOP_SEGMENT_SKIP(); \

#define VPID_DUPLICATE_VREG(reg_num, idx_sew) \
reg_t index[PID_SEGMENT_NUM]; \
 for (reg_t i = 0; i < PID_SEGMENT_NUM && P.VU.vl->read() != 0; ++i) { \
  switch (idx_sew) { \
    case e8: \
      index[i] = P.VU.elt<uint8_t>(reg_num, i); \
      break; \
...
#define VPID_STORE(elt_width, is_mask_ldst) \
  const reg_t vl = is_mask_ldst ? ((P.VU.vl->read() + 7) / 8) : P.VU.vl->read(); \
  const reg_t stride = RS1; \
  const reg_t rd_num = insn.rd(); \
  const reg_t rs2_num = insn.rs2(); \
  ...
  for (reg_t i = 0; i < vl; ++i) { \
    ...
    for (reg_t fn = 0; fn < nf; ++fn) { \
      ...
      switch(P.VU.vsew) { \
        case e8: { \
          auto val = P.VU.elt<uint8_t>(rd_num + fn, i, true); \
          MMU.store<uint8_t>(index[fn] + (stride * i), val); \
          break; } \
        case e16: { \
          auto val = P.VU.elt<uint16_t>(rd_num + fn, i, true);\
          MMU.store<uint16_t>(index[fn] + (stride * i), val); \
          break; } \
        case e32: { \
          auto val = P.VU.elt<uint32_t>(rd_num + fn, i, true);\
          MMU.store<uint32_t>(index[fn] + (stride * i), val); \
          break; } \
        case e64: { \
          auto val = P.VU.elt<uint64_t>(rd_num + fn, i, true);\
          MMU.store<uint64_t>(index[fn] + (stride * i), val); \
          break; } \
      } \
    } \
  } \
